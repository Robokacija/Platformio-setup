...
{
    /// @brief proba2
    float x; /// @brief probax pozicija
    float y; // y pozicija
    uint8_t xSpeed;
    uint8_t ySpeed;
    uint8_t width;
    uint8_t height;
    uint16_t *buffer; // adresa nacrtane slike (npr. buffer = (uint16_t*) moja_slika)
};
...
public:
    ...
    Sprite create_text(char *chars, uint16_t color, uint8_t font_size);
    void set_bitmap(uint8_t x, uint8_t y, Sprite *buffer);
...
private:
    ...
    void read_char(Sprite *bitmap, int x, int y, float w, float h, uint8_t *buffer, float font_width, float font_height, bool elongate, uint16_t color);
